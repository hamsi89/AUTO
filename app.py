...
def load_excel_master():
    """기존 엑셀 시트에서 최초 마스터 품목 정보를 파싱하는 함수 (와인 추가 보완)"""
    sheets_to_try = {
        "원재료": ["원재료", "원재료(간략)"],
        "부자재": ["부자재", "부자재(간략)"],
        "디저트&완제품": ["디저트&완제품", "디저트&완제품(간략)"],
        "와인": ["와인_7월수불대장", "와인"] # 🎯 마스터 로드 대상에 와인 시트 추가
    }
            ...
            for cat, standard_names in sheets_to_try.items():
                ...
                if target_sheet:
                    ...
                    for col in df.columns:
                        # 🎯 와인 시트의 특수 헤더 '품목이름(구분/발주처)'까지 포함해서 무조건 인식하도록 보강
                    ...
                    if name_col:
                        ...
                        stock_col = [c for c in df.columns if '재고' in c or '현재고' in c]
    ...
    return pd.DataFrame([
        {"품목명": "커피(퍼늘-마스터피스)", "대분류": "원재료", "유통기한": "2027-04-13", "엑셀기본재고": 54},
